...
namespace OpenNLP.Tools.NameFind
{
	...
	public class EnglishNameFinder 
	{
        ...
        public static string[] NameTypes = { "person", "organization", "location", "date", "time", "percentage", "money", "make", "model", "make-model" };
		...
		public string TagNames(string[] models, string data)
		{
			CreateModels(models);
			return ProcessText(models, data);
		}

		public string TagNames(string[] models, Parse data)
		{
			CreateModels(models);
			return ProcessParse(models, data);
		}

		public IEnumerable<(string name, double probability)> GetNamesWithProbabilities(string model, string data)
		{
			CreateModels(new string[] { model });
			return ParseNamesWithProbabilities(model, data);
		}
		...
		private Dictionary<string, string> CreatePreviousTokenMap()
		{
		...
		private void ClearPreviousTokenMap(Dictionary<string, string> previousTokenMap)
		{
		...
		private void UpdatePreviousTokenMap(Dictionary<string, string> previousTokenMap, string[] tokens, string[] finderTags)
        {
			for (int currentToken = 0, tokenCount = tokens.Length; currentToken < tokenCount; currentToken++)
			{
				previousTokenMap[tokens[currentToken]] = finderTags[currentToken];
			}
		}
		...
		private string ProcessParse(string[] models, Parse lineParse)
		{
			...
			for (int currentFinder = 0, finderCount = models.Length; currentFinder < finderCount; currentFinder++)
			{
				...
				var probabilities = finder.GetProbabilities();
			}
		...
		private IEnumerable<(string name, double probability)> ParseNamesWithProbabilities(string model, string line)
		{
			var previousTokenMap = CreatePreviousTokenMap();
			var namesWithProbabilities = new List<(string, double)>();

			if (line.Length == 0)
			{
				ClearPreviousTokenMap(previousTokenMap);
			}
			else
			{
				Span[] spans = TokenizeToSpans(line);
				...
				MaximumEntropyNameFinder finder = mFinders[model];
				var finderTags = finder.Find(tokens, previousTokenMap);
				var probabilities = finder.GetProbabilities();

				UpdatePreviousTokenMap(previousTokenMap, tokens, finderTags.ToArray());
				for (int currentToken = 0, tokenCount = tokens.Length; currentToken < tokenCount; currentToken++)
				{

					if (finderTags[currentToken] == MaximumEntropyNameFinder.Start)
                    {
						var name = line.Substring(spans[currentToken].Start, (spans[currentToken].End) - (spans[currentToken].Start));
						var probability = probabilities[currentToken];
						namesWithProbabilities.Add((name, probability));
					}

					if (finderTags[currentToken] == MaximumEntropyNameFinder.Continue)
					{
						var name = line.Substring(spans[currentToken].Start, (spans[currentToken].End) - (spans[currentToken].Start));
						var probability = probabilities[currentToken];

						var previousNameWithProbability = namesWithProbabilities.ElementAt(namesWithProbabilities.Count - 1);
						if (previousNameWithProbability.Item1 != null)
                        {
							previousNameWithProbability.Item1 += " " + name;
							previousNameWithProbability.Item2 = Math.Max(previousNameWithProbability.Item2, probability);
						}
						namesWithProbabilities[namesWithProbabilities.Count - 1] = previousNameWithProbability;
					}
				}
			}
			return namesWithProbabilities;
		}
